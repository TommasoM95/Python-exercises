...
class State():
    def __init__(self, name=None, method=None, activated=False):
        self.name = name
        self.method = method
        self.active = activated                                                                             ######################  Different names
...
#highway exit
STRAIGHT_DIST_TO_EXIT_HIGHWAY = 0.8 #[m] go straight for this distance in order to exit the highway
...
DIST_4S = 0.05 #[m] semaphore distance to perform the 4th part of s manouver
...
# OR between XOR and NOR => TRUE if all F, all T or if only one flag is T       #       Check the logic, why assert for 1 flag T or all the same?           ###################################################################################################################################
...
TAILING_DISTANCE = 0.25 #[m] distance to keep from the vehicle while tailing             #      Why here? tailing is for overtaking   ##############################################################################
#overtake static car
